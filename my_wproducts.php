...
$sQuery = ("select s.*,c.Title as ctitle from Store s inner join StoreCategories c on c.ID=s.categoryID where s.type=1 and  s.status=0 and s.userID=$profileID  order by $order ");	
...
if($profileID == $_SESSION['memberID'])
{
	...
	$sQuery = ("select s.*,c.Title as ctitle from Store s inner join StoreCategories c on c.ID=s.categoryID where s.type=1 and  s.status=1 and s.userID=$profileID  order by $order ");		
	$rElems = mysql_query( $sQuery );		
	$iNums = mysql_num_rows($rElems);	
	$count = (int)($iNums/$iDivis);
	if(($iNums/$iDivis) > (int)($iNums/$iDivis)) $count++;
...
show_smarty_template('my_wproducts');
?>
