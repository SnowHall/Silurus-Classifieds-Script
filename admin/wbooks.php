...
if(isset($_GET['del']) && intval($_GET['ID']) > 0)
{
	...
	header("location: /admin/wbooks.php");
}
$action = $_GET['action'];
...
function getList( $iArticleID = '' )
{
	...
	$ret = '<a href="/admin/wbooks.php">All by create date</a><br><form method="GET">Search by title: <input type="text" name="searchID"> <input type=submit name=find value="Find"></form><br><br>
	<table width=100%>';	
	...
	while($book = mysql_fetch_assoc($rElems))
	{
		...
		<td valign=top>&nbsp;&nbsp;&nbsp;<a href="javascript:if(confirm(\'Delete product?\')) window.location=\'/admin/wbooks.php?del&ID='.$book['ID'].'\';">Delete</a></td></tr>';
	}
	$ret.='</table><br><br>'.$nav;
	return  $ret;
}
