...
if( isset($_POST['addcategory']) && trim($_POST['Name']) != '')
{
	...
	header("location: /admin/prop_seller.php");
}
...
function getTree()
{
	...
	while ( $aSelect = mysql_fetch_assoc( $rSelect ))
	{
		...
		<td><a href="javascript:if(confirm(\'Are you shure delete '.$aSelect['Title'].'? All products and properties will be delete.\')) window.location=\'/admin/prop_seller.php?action=delcategory&id='.$aSelect['ID'].'\';">Delete</a> </td></tr>';
		...
		while ( $aSelect2 = mysql_fetch_assoc($rSelect2))
		{
			...
			<td><a href="/admin/prop_seller.php?action=editprop&id='.$aSelect2['ID'].'">Edit</a> </td><td><a href="javascript:if(confirm(\'Are you shure delete '.$aSelect2['Name'].'?\')) window.location=\'/admin/prop_seller.php?action=delprop&id='.$aSelect2['ID'].'\';">Delete</a> </td></tr>';
		}
	}
	$ret .= "</table>";	
	return $ret;	
