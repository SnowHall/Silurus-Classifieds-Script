...
function getList( $iArticleID = '' )
{
	...
	$ret = '<a href="/admin/city.php?action=add">Add City</a><br><br>
	<table width="100%">';	
	...
	while($a = mysql_fetch_assoc($q))
	{
		...
		<td valign=top><a href="/admin/city.php?action=edit&ID='.$a['ID'].'">Edit</a> &nbsp;&nbsp;&nbsp;<a href="javascript:if(confirm(\'Delete City?\')) window.location=\'/admin/city.php?del&ID='.$a['ID'].'\';">Delete</a></td></tr>';
	}
	$ret.='</table>';
	return  $ret;
}
...
function getEdit( $iArticleID = '' )
{
	...
	$ret .= '<div class="navigationLinks">' . "\n";
		$ret .= '<span>' . "\n";
			$ret .= '<a href="' . $site['url_admin'] . 'city.php">' . "\n";
				$ret .= 'All City' . "\n";
			$ret .= '</a>' . "\n";
		$ret .= '</span>' . "\n";
	...
	$ret .= '<div class="articlesFormBlock">' . "\n";
		$ret .= '<form method="post" action="' . $site['url_admin'] . 'city.php" onsubmit="return checkForm();" enctype="multipart/form-data">' . "\n";
			$ret .= '<div>' . "\n";
				$ret .= '<b>Title</b>' . "\n";
			$ret .= '</div>' . "\n";
			$ret .= '<div>' . "\n";
