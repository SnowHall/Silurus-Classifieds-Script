...
$smarty->assign("site_title",  "Manage Product Properties (for buyer) :: Admin Panel :: ".$gConfig['site_title']);
$smarty->assign("page_header_text",  "Manage Product Properties (for buyer)");

$action = $_GET['action'];

if( isset($_POST['addcategory']) && trim($_POST['Name']) != '')
{
	mysql_query( "insert into StoreCategories set Title='".trim($_POST['Name'])."', Type=1");
	header("location: {$gConfig['site_url']}admin/prop_buyer.php");
}
...
function getTree()
{
	...
	$ret = '<form method="post" action="" enctype="multipart/form-data">' . "\n";
	$ret .= '<div>' . "\n";
		$ret .= '<b>Add Category for Buyer</b>' . "\n";
	$ret .= '</div>' . "\n";
	$ret .= '<div>' . "\n";
	...
	$ret .= "<table width=100%>";
	$rSelect = mysql_query( "select * from StoreCategories where Type=1 order by Title" );
	while ( $aSelect = mysql_fetch_assoc( $rSelect ))
	{
		$ret .= '<tr><td width=50%><b>'.$aSelect['Title'].'</b></td><td> </td>
		<td><a href="admin/prop_buyer.php?action=addprop&id='.$aSelect['ID'].'">Add property</a></td>
		<td><a href="admin/prop_buyer.php?action=editcategory&id='.$aSelect['ID'].'">Edit</a> </td>
		<td><a href="javascript:if(confirm(\'Are you shure delete '.$aSelect['Title'].'? All products and properties will be delete.\')) top.window.location=\''.$gConfig['site_url'].'admin/prop_buyer.php?action=delcategory&id='.$aSelect['ID'].(isset($_GET['demo'])?'&demo=1':'').'\';">Delete</a> </td></tr>';
		...
		while ( $aSelect2 = mysql_fetch_assoc($rSelect2))
		{
			...
			<td><a href="admin/prop_buyer.php?action=editprop&id='.$aSelect2['ID'].'">Edit</a> </td><td><a href="javascript:if(confirm(\'Are you shure delete '.$aSelect2['Name'].'?\')) top.window.location=\'/admin/prop_buyer.php?action=delprop&id='.$aSelect2['ID'].'\';">Delete</a> </td></tr>';
		}
	}
	$ret .= "</table>";
	return $ret;
