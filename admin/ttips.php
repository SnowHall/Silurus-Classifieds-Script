...
$smarty->assign("site_title",  "Manage Technical Tips :: Admin Panel :: ".$gConfig['site_title']);
$smarty->assign("page_header_text",  "Manage Technical Tips");
...
function getEdit( $iArticleID = '' )
{
	...
	$articleQuery = "
		...
		SELECT * from TTips WHERE `ID` = '$iArticleID';
	";
	...
	$ret .= '<div class="navigationLinks">' . "\n";
		$ret .= '<span>' . "\n";
			$ret .= '<a href="' . $site['url_admin'] . 'ttips.php">' . "\n";
				$ret .= 'All tips' . "\n";
			$ret .= '</a>' . "\n";
		$ret .= '</span>' . "\n";
	...
	$ret .= '<div class="articlesFormBlock">' . "\n";
		$ret .= '<form method="post" action="' . $site['url_admin'] . 'ttips.php" onsubmit="return checkForm();" enctype="multipart/form-data">' . "\n";
			$ret .= '<div>' . "\n";
				$ret .= '<b>Title</b>' . "\n";
			$ret .= '</div>' . "\n";
			$ret .= '<div>' . "\n";
